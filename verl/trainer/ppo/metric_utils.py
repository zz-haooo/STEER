    ...
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                # Filter out None values and ensure all values are numerical
                filtered_vals = []
                for v in var_vals:
                    if v is not None:
                        try:
                            # Try to convert to float to ensure it's numerical
                            float_val = float(v)
                            filtered_vals.append(float_val)
                        except (ValueError, TypeError):
                            # Skip non-numerical values
                            continue
                
                if not filtered_vals:
                    # Skip if no valid numerical values
                    continue
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        [(bon_mean, bon_std), (won_mean, won_std)] = bootstrap_metric(data=filtered_vals, subset_size=n, reduce_fns=[np.max, np.min], seed=seed)
                        metric[f"best@{n}/mean"], metric[f"best@{n}/std"] = bon_mean, bon_std
                        metric[f"worst@{n}/mean"], metric[f"worst@{n}/std"] = won_mean, won_std
                        if var2vals.get("pred", None) is not None:
                            # Filter pred values corresponding to non-None vals
                            pred_vals = var2vals["pred"]
                            filtered_pred_vals = [pred_vals[i] for i, v in enumerate(var_vals) if v is not None]
                            if len(filtered_pred_vals) == len(filtered_vals):
                                vote_data = [{"val": val, "pred": pred} for val, pred in zip(filtered_vals, filtered_pred_vals)]
