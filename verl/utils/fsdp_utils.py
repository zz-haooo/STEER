...
if version.parse(torch.__version__) >= version.parse("2.6"):
    ...
    fully_shard, MixedPrecisionPolicy, FSDPModule, CPUOffloadPolicy = None, None, None
...
def offload_fsdp_model_to_cpu(model: FSDP, empty_cache: bool = True):
    ...
    # First synchronize CUDA operations to ensure previous operations complete
    ...
    for handle in model._all_handles:
        ...
        try:
            # Use synchronous method to move tensors, avoiding async operation conflicts
            handle.flat_param_to(torch.device("cpu"), non_blocking=False)
        except RuntimeError as e:
            if "CUDA" in str(e):
                # If CUDA error occurs, clear cache first then retry
                torch.cuda.empty_cache()
                try:
                    # Try again using synchronous method
                    handle.flat_param_to(torch.device("cpu"), non_blocking=False)
                except RuntimeError as e2:
                    print(f"Warning: Failed to offload FSDP parameter to CPU: {e2}")
                    # If still fails, skip this parameter
                    continue
            else:
            ...
            for key, value in state.items():
                if isinstance(value, torch.Tensor):
                    try:
                        ...
                        # If CUDA error occurs, try clearing cache then retry
                        if "CUDA" in str(e):
                            torch.cuda.empty_cache()
                            try:
                                # Try again, this time using synchronous method
                                state[key] = value.to("cpu", non_blocking=False)
                            except RuntimeError as e2:
                                # If still fails, log error but continue execution
                                print(f"Warning: Failed to offload tensor {key} to CPU: {e2}")
                                # Can choose to skip this tensor or use other strategies
                                continue
                        else:
                            # Re-raise non-CUDA errors
                            raise e
